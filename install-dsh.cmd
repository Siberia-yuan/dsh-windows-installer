...
goto :post_target
:target_exists
if exist "%TARGET%\.git" ( echo   [info] existing repository found - will update instead of clone. & set "EXISTING_REPO=1" & call :log "install dir exists - update mode" & goto :post_target )
...
if defined EXISTING_REPO goto :repo_update
echo   cloning via mirror %GH_PROXY% ...
call :log "git clone via mirror"
git clone %GH_PROXY% "%TARGET%" 2>nul
if errorlevel 1 goto :clone_retry
goto :clone_check
:clone_retry
...
:clone_check
if exist "%TARGET%\package.json" goto :clone_ok
echo   [FAIL] clone incomplete.
goto :fail
:clone_ok
echo   [ok] source fetched.
call :log "source cloned OK"
goto :after_clone
:repo_update
pushd "%TARGET%"
...
goto :after_clone
:clone_failed
echo   [FAIL] could not clone the repository. Possible causes:
echo          - network blocked (try another network / enable a proxy)
echo          - the target directory is not empty and not a git repo:
echo            delete "%TARGET%" first, or use -d with a new directory
goto :fail
:after_clone
...
if errorlevel 1 ( echo   [FAIL] pnpm install failed. Common causes: network to registry.npmmirror.com, or a postinstall script error. See output above; retry after fixing. & call :log "pnpm install FAILED" & popd & goto :fail )
...
> "%TARGET%\start-dsh.cmd" echo @echo off
>> "%TARGET%\start-dsh.cmd" echo rem DeepSeek Harness launcher - generated by installer
>> "%TARGET%\start-dsh.cmd" echo cd /d "%%~dp0"
>> "%TARGET%\start-dsh.cmd" echo rem Smart launcher: open browser if server already up, else start it
>> "%TARGET%\start-dsh.cmd" echo rem No hard-coded paths: node is located at run time ^(PATH first,
>> "%TARGET%\start-dsh.cmd" echo rem then the self-contained copy the installer downloaded, if any^).
>> "%TARGET%\start-dsh.cmd" echo netstat -ano ^| findstr ":3080" ^| findstr "LISTENING" ^>nul 2^>^&1
>> "%TARGET%\start-dsh.cmd" echo if not errorlevel 1 goto :already
>> "%TARGET%\start-dsh.cmd" echo echo DeepSeek Harness server not running, starting ...
>> "%TARGET%\start-dsh.cmd" echo set "NODE_CMD="
>> "%TARGET%\start-dsh.cmd" echo where node ^>nul 2^>^&1 ^&^& set "NODE_CMD=node"
>> "%TARGET%\start-dsh.cmd" echo if not defined NODE_CMD if exist "!NODE_BIN!" set "NODE_CMD=!NODE_BIN!"
>> "%TARGET%\start-dsh.cmd" echo if not defined NODE_CMD ^( echo [ERROR] Node.js not found in PATH. Please install Node.js 22.19+ and retry. ^& pause ^& exit /b 1 ^)
>> "%TARGET%\start-dsh.cmd" echo "%%NODE_CMD%%" --import tsx/esm apps/cli/src/bin.ts web
>> "%TARGET%\start-dsh.cmd" echo powershell -NoProfile -ExecutionPolicy Bypass -Command "for($i=0;$i -lt 30;$i++){try{$r=Invoke-WebRequest -Uri 'http://127.0.0.1:3080' -UseBasicParsing -TimeoutSec 2;if($r.StatusCode -eq 200){Start-Process 'http://127.0.0.1:3080';exit}}catch{};Start-Sleep 2}"
>> "%TARGET%\start-dsh.cmd" echo goto :eof
>> "%TARGET%\start-dsh.cmd" echo :already
>> "%TARGET%\start-dsh.cmd" echo echo DeepSeek Harness server already running, opening browser ...
>> "%TARGET%\start-dsh.cmd" echo start "" http://127.0.0.1:3080
>> "%TARGET%\start-dsh.cmd" echo goto :eof
...
if exist "%DESKTOP%\DeepSeek Harness.lnk" ( echo   [ok] shortcut created: "%DESKTOP%\DeepSeek Harness.lnk" & call :log "desktop shortcut created" ) else ( echo   [WARN] desktop shortcut was not created - no problem. Start it: open "%TARGET%" and double-click start-dsh.cmd, browser opens http://127.0.0.1:3080. & call :log "desktop shortcut NOT created - start via start-dsh.cmd" )
:after_shortcut
...
echo.
echo   ============================================================
if defined DRY ( echo   [DRY-RUN PREVIEW COMPLETE - nothing was changed] ) else ( echo   ** INSTALL SUCCESS - DeepSeek Harness is ready to use! ** )
echo   ============================================================
...
:do_patch
  ...
  if not exist "%TARGET%\.npmrc" ( echo %NPM_REG%> "%TARGET%\.npmrc" ) else ( findstr /c:"registry.npmmirror.com" "%TARGET%\.npmrc" >nul 2>&1 || echo %NPM_REG%>> "%TARGET%\.npmrc" )
...
rem ============================================================== git tool
:ensure_git
  if defined FORCE_DL if defined DRY ( echo   [dry-run] --force-download: will fetch bundled PortableGit to tools\git. & exit /b 0 )
  if defined FORCE_DL goto :download_git
  where git >nul 2>&1
  if not errorlevel 1 ( for /f "delims=" %%v in ('git --version 2^>nul') do set "GIT_VER=%%v" & echo   [ok] git: !GIT_VER! ^(system git - no download^) & exit /b 0 )
...
:download_git
  ...
  if exist "%LOCAL_GIT%\cmd\git.exe" ( set "GIT_BIN=%LOCAL_GIT%\cmd" & set "PATH=%LOCAL_GIT%\cmd;!PATH!" & echo   [ok] using bundled PortableGit - already downloaded, no download needed. & exit /b 0 )
  ...
  del /q "%GIT_SFX%" 2>nul
  if exist "%LOCAL_GIT%\cmd\git.exe" ( set "GIT_BIN=%LOCAL_GIT%\cmd" & set "PATH=%LOCAL_GIT%\cmd;!PATH!" & for /f "delims=" %%v in ('"%LOCAL_GIT%\cmd\git.exe" --version 2^>nul') do set "GIT_VER=%%v" & echo   [ok] bundled git ready: !GIT_VER! & exit /b 0 )
  echo   [FAIL] PortableGit extraction failed.
  exit /b 1

rem ============================================================= node tool
:ensure_node
  if defined FORCE_DL if defined DRY ( echo   [dry-run] --force-download: will fetch bundled Node 22 to tools\. & exit /b 0 )
  if defined FORCE_DL goto :download_node
  where node >nul 2>&1
  if errorlevel 1 if defined DRY ( echo   [dry-run] node not found - will download bundled Node 22 to tools\. & exit /b 0 )
  ...
  if not errorlevel 1 ( echo   [ok] node: !NODE_VER! ^(system node, meets >=22.19 - no download^) & exit /b 0 )
...
:download_node
  ...
  if exist "%LOCAL_NODE%\node.exe" ( set "NODE_BIN=%LOCAL_NODE%\node.exe" & set "PATH=%LOCAL_NODE%;!PATH!" & echo   [ok] using bundled node - already downloaded, no download needed. & exit /b 0 )
  ...
  if exist "%LOCAL_NODE%\node.exe" ( set "NODE_BIN=%LOCAL_NODE%\node.exe" & set "PATH=%LOCAL_NODE%;!PATH!" & echo   [ok] self-contained node downloaded. & exit /b 0 )
